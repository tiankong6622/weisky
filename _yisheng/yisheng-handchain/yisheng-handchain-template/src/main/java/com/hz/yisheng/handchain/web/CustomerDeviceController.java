...
public class CustomerDeviceController extends BaseController {
	 ...
	 * @return
	 */
	@RequestMapping("/getDevicesUse/{id}")
	public String getDevicesUse(@PathVariable("id") Long id,HttpServletRequest request,HttpServletResponse response,Model model){
		CustomerDevice customerDevice = customerDeviceBO.getCustomerDeviceById(id);
		List<CustomerDevice> device = customerDeviceBO.deviceUse(customerDevice.getNumber());
		model.addAttribute("device", device);
		return "/customer/historyDeviceUse";
	}
